...
using DotnetPackaging.Exe.Installer.Flows.Uninstallation.Wizard;
using Serilog;
using Zafiro.Avalonia.Dialogs.Implementations;
using Zafiro.Avalonia.Dialogs.Wizards.Slim;

namespace DotnetPackaging.Exe.Installer.Flows.Uninstallation;
...
internal static class Uninstaller
{
    public static async Task Launch()
    {
        ...
        try
        {
            Log.Information("Uninstallation launched");
            ...
            Log.Information("Self-destruct triggered");
        }
        catch (Exception ex)
        {
            Log.Fatal(ex, "Uninstallation failed");
            TryWriteCrashLog(ex);
            throw;
        }
    }
    ...
    private static IInstallerPayload ResolvePayload()
    {
        ...
        {
            Log.Information("Using metadata.json from installation directory");
            return payloadResult.Value;
        }

        Log.Warning("metadata.json not found: {Error}. Using fallback payload", payloadResult.Error);
    ...
    private static async Task<string> ResolveWindowTitle(IInstallerPayload payload)
    {
        ...
        {
            Log.Warning("Metadata load failed: {Error}", metaResult.Error);
        }
